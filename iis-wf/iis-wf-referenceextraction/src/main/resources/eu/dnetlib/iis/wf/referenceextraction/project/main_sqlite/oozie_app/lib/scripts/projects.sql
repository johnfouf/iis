...
select * from (select id, textwindow2s(keywords(filterstopwords(text)),15,4,10,"\bINCa|French National Cancer Institute") from pubs) where regexprmatches("fund|grant|inserm|dgos|support|project|fondation|program|financial|plbio|acknowledge|thank|award|finance|financ√©e|subvention|bourse|aide|soutien|remerci|recipient|\bprojet\b",lower(j2s(prev,middle,next))) and not regexprmatches("instituto nacional câncer|brazil|janeiro|mexico|méxico|instituto nacional cancerología", lower(j2s(prev,middle,next))) and not regexprmatches("incan(\D|\s)",lower(j2s(prev,middle,next)));
